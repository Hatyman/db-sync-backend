...
using MccSoft.DbSyncApp.ComponentTests.Infrastructure;
using MccSoft.DbSyncApp.Http;
...
namespace MccSoft.DbSyncApp.ComponentTests
{
    public class TestBase : Testing.TestBase, IDisposable
    {
        ...
        protected TestBase()
        {
            ...
            var db = ResolveFromTestScope<DbSyncAppDbContext>();
        ...
        protected void WithDbContext(Action<DbSyncAppDbContext> action)
        {
            using var scope = TestServer.Host.Services.CreateScope();
            var db = scope.ServiceProvider.GetRequiredService<DbSyncAppDbContext>();
            action(db);
        }

        protected T WithDbContext<T>(Func<DbSyncAppDbContext, T> action)
        {
            using var scope = TestServer.Host.Services.CreateScope();
            var db = scope.ServiceProvider.GetRequiredService<DbSyncAppDbContext>();
            return action(db);
