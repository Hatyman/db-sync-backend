...
const replacements = [
  { find: "DbSyncApp", replace: projectName },
  { find: "dbsyncapp", replace: projectName.toLowerCase() },
  {
    find: "dbSyncApp",
    replace:
...
function changeFrontendPortNumber(port) {
  replace({
    ...
    paths: ["./webapi/src/MccSoft.DbSyncApp.App/Startup.cs"],
    silent: true,
  });
  replace({
    regex: / PORT=\d+ /,
...
function changeBackendPortNumber(port) {
  replace({
    ...
    paths: [
      "./webapi/src/MccSoft.DbSyncApp.App/appsettings.Development.json",
    ],
    silent: true,
  });
  replace({
...
function changePasswordsInAppsettings() {
  replace({
    ...
    paths: ["./webapi/src/MccSoft.DbSyncApp.App/appsettings.json"],
    silent: true,
  });
  replace({
